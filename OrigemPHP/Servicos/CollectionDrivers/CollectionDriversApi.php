...
class CollectionDriversApi extends BaseController
{
    public static function listar($request)
    {
        ...
        $query = DB::table('collection_drivers')->select('id', 'code', 'name', 'description', 'enabled', 'created_at', 'updated_at');
        $search = trim((string) ($params['search'] ?? ''));
        if ($search !== '') {
            $like = '%' . $search . '%';
            $query->where(function ($q) use ($like) { $q->where('code', 'ilike', $like)->orWhere('name', 'ilike', $like)->orWhere('description', 'ilike', $like); });
        }
    ...
    public static function listarSimples($request)
    {
        Permissao::proteger('ieds.view');
        $params = $request->getQueryParams();
        $query = DB::table('collection_drivers')->select('id', 'code', 'name', 'enabled')->where(function ($q) use ($params) { $q->where('enabled', true); if (!empty($params['include_id'])) $q->orWhere('id', $params['include_id']); });
    ...
    public static function criar($request)
    {
        ...
        } catch (\Throwable $e) { return ['erro' => 1, 'mensagem' => self::isDuplicate($e) ? 'Já existe um Driver com este código.' : 'Não foi possível criar o Driver de Coleta.']; }
    }

    public static function atualizar($request, $id)
    {
        Permissao::proteger('ieds.manage');
        $current = self::find($id); if (!$current) return ['erro' => 1, 'mensagem' => 'Driver de Coleta não encontrado.'];
        $data = $request->getPostVars(); $error = self::validate($data, $id);
        if ($error) return ['erro' => 1, 'mensagem' => $error];
        if (trim($data['code']) !== $current->code && self::isReferenced($id)) return ['erro' => 1, 'mensagem' => 'O código não pode ser alterado enquanto o Driver estiver associado a Templates ou IEDs.'];
        try {
        ...
        } catch (\Throwable $e) { return ['erro' => 1, 'mensagem' => self::isDuplicate($e) ? 'Já existe um Driver com este código.' : 'Não foi possível atualizar o Driver de Coleta.']; }
    }

    public static function alterarStatus($request, $id)
    {
        Permissao::proteger('ieds.manage');
        if (!self::find($id)) return ['erro' => 1, 'mensagem' => 'Driver de Coleta não encontrado.'];
        $data = $request->getPostVars(); if (!array_key_exists('enabled', $data)) return ['erro' => 1, 'mensagem' => 'Informe o novo status.'];
        $enabled = self::booleanValue($data['enabled']); if (!$enabled && self::isReferenced($id)) return ['erro' => 1, 'mensagem' => 'O Driver não pode ser desativado enquanto estiver associado a Templates ou IEDs.'];
    ...
    private static function find($id) { return DB::table('collection_drivers')->select('id', 'code', 'name', 'description', 'enabled', 'created_at', 'updated_at')->where('id', $id)->first(); }
    private static function data(array $data, bool $creating = false): array { return ['code' => trim($data['code']), 'name' => trim($data['name']), 'description' => trim((string) ($data['description'] ?? '')) ?: null, 'enabled' => self::booleanValue($data['enabled'] ?? true), 'updated_at' => date('Y-m-d H:i:s')] + ($creating ? ['created_at' => date('Y-m-d H:i:s')] : []); }
    private static function validate(array $data, $id = null)
    {
        $code = trim((string) ($data['code'] ?? '')); $name = trim((string) ($data['name'] ?? ''));
        if ($code === '') return 'Informe o código técnico do Driver.';
        if (!preg_match('/^[a-z0-9_-]+$/', $code)) return 'O código deve conter apenas letras minúsculas, números, hífen ou sublinhado.';
        if ($name === '') return 'Informe o nome do Driver de Coleta.';
        $query = DB::table('collection_drivers')->where('code', $code); if ($id !== null) $query->where('id', '<>', $id); if ($query->exists()) return 'Já existe um Driver com este código.';
        return null;
    }
    private static function isReferenced($id): bool { return DB::table('ied_templates')->where('default_driver_id', $id)->exists() || DB::table('ieds')->where('driver_override_id', $id)->exists(); }
    ...
    private static function agentStatuses(): array
    {
        ...
        if ($agentUrl === '') return $result;
        try {
            $curl = new Curl();
            $curl->setOpt(CURLOPT_CONNECTTIMEOUT, 2);
            $curl->setTimeout(5);
            $response = $curl->get(rtrim($agentUrl, '/') . '/api/v1/capabilities');
            if ($curl->curlError || (int) $curl->httpStatusCode !== 200) return $result;
            if ($response instanceof \stdClass) $response = (array) $response;
            if (!is_array($response) || !isset($response['drivers'])) return $result;
            $drivers = $response['drivers'];
            if ($drivers instanceof \stdClass) $drivers = (array) $drivers;
            if (!is_array($drivers)) return $result;
            foreach ($drivers as $driver) {
                if ($driver instanceof \stdClass) $driver = (array) $driver;
                if (!is_array($driver) || !is_string($driver['code'] ?? null) || !in_array($driver['status'] ?? null, ['implemented', 'partial'], true)) return $result;
