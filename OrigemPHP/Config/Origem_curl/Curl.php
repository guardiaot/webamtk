...
namespace OrigemPHP\Config\Origem_curl;

use OrigemPHP\Config\Origem_curl\ArrayUtil;
use OrigemPHP\Config\Origem_curl\Decoder;
use OrigemPHP\Config\Origem_curl\Url;
...
class Curl
{
    ...
    {
        $this->jsonDecoder = 'OrigemPHP\Config\Origem_curl\Decoder::decodeJson';
    ...
    {
        $this->xmlDecoder = 'OrigemPHP\Config\Origem_curl\Decoder::decodeXml';
    ...
    public function setDefaultDecoder($mixed = 'json')
    {
        if ($mixed === false) {
            ...
            if ($mixed === 'json') {
                $this->defaultDecoder = 'OrigemPHP\Config\Origem_curl\Decoder::decodeJson';
            } elseif ($mixed === 'xml') {
                $this->defaultDecoder = 'OrigemPHP\Config\Origem_curl\Decoder::decodeXml';
            }
        }
    }
    ...
    {
        $user_agent = 'PHP-Curl-Class/' . self::VERSION . ' (+https://ws.ksiseguros.com.br/paginas/curl.html)';
    ...
    * @param $proxy - O proxy HTTP para o túnel de solicitações. Pode incluir o número da porta.
     ...
     * @param  $raw_headers
     *
     * @return OrigemPHP\Config\Origem_curl\CaseInsensitiveArray
     */
