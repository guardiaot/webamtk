...
import IEDTypeOffcanvas from "./IEDTypeOffcanvas";

export default function IEDTypes() {
    ...
    confirmation.current = toast.custom(
      (t) => (
        <div
          ...
          style={{ minWidth: 320 }}
        >
          <div className="fw-semibold mb-1">{action} Tipo de IED?</div>
              ...
              onClick={async () => {
                ...
                try {
                  const response = await fetch(
                    `/api/v1/ied-types/${row.id}/status`,
                    {
                      method: "POST",
                      headers: {
    ...
    <LayoutAdmin>
      <div className="container-fluid py-4">
        <div className="d-flex justify-content-between align-items-center mb-4">
          <div>
            <h2 className="mb-1">Tipos de IEDs</h2>
            <div className="text-muted">
              Categorias funcionais dos equipamentos
            </div>
          </div>
          {canManage && (
            <button
        ...
        <DataTable
          ref={table}
          title="Tipos de IED cadastrados"
          ajax="/api/v1/ied-types"
          columns={columns}
          filters={filters}
